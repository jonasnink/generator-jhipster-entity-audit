...
// INICIO [MPRO] (CODIGO INCLUIDO)
var hasDomainCustom = false;
// FIM [MPRO]
    ...
    server: [
        {
            ...
            templates: [
                // INICIO [MPRO] (CODIGO ALTERADO PARA GERAR/SOBRESCREVER APENAS AS CLASSES DO DIRETÓRIO generated )
                {
                    file: 'package/domain/Entity.java',
                    renameTo: generator => `${generator.packageFolder}/domain/jhi/${generator.asEntity(generator.entityClass)}Jhi.java`
                },
                // FIM [MPRO]
                {
        ...
        // INICIO [MPRO] (CODIGO INCLUIDO)
        {
            condition: generator => !hasDomainCustom,
            path: SERVER_MAIN_SRC_DIR,
            templates: [
                {
                    file: 'package/domain/Entity_custom.java',
...
function writeFiles() {
    return {
        ...
        writeServerFiles() {
            // INICIO [MPRO] (CODIGO INCLUIDO)
            if (fs.existsSync(`${SERVER_MAIN_SRC_DIR}${this.packageFolder}/domain/${this.entityClass}.java`)) {
                hasDomainCustom = true;              
            }
            else{
                hasDomainCustom = false;                
            }
            // FIM [MPRO]
            ...
            if (this.databaseType === 'sql') {
                ...
                if (['ehcache', 'infinispan'].includes(this.cacheProvider) && this.enableHibernateCache) {
                    ...
                    // INICIO [MPRO] (CODIGO INCLUIDO)
                    // this.addEntityToCache(
                    //     this.asEntity('jhi.'+this.entityClass+'Jhi'),
                    //     this.relationships,
                    //     this.packageName,
                    //     this.packageFolder,
                    //     this.cacheProvider
        ...
        writeEnumFiles() {
            this.fields.forEach(field => {
                if (field.fieldIsEnum === true) {
                    ...
                    if (!this.skipServer) {
                        // INICIO [MPRO] (CODIGO ALTERADO PARA GERAR/SOBRESCREVER APENAS SE NÃO EXISTIR O ENUM.)
                        if(!fs.existsSync(`${SERVER_MAIN_SRC_DIR}${this.packageFolder}/domain/enumeration/${fieldType}.java`)) {
                            this.template(
